...
namespace Test
{
    ...
    public class ConversationTest : MonoBehaviour
    {
        ...
        [System.Serializable]
        public class JsonResponseDataDummy
        {
            // llama.cppのcompletionエンドポイントからの代表的なレスポンス構造
            public string content; // 生成されたテキスト
            public int tokens_predicted; // 生成されたトークン数
            public bool stopped_eos; // 終了トークンで停止したか
            // 他のフィールド（tokens_evaluated, timingなど）は必要に応じて追加
        }
        ...
        private async Task<string> GetLLMResponseDummy(string text)
        {
            ...
            var payload = new
            {
                prompt = text,
                n_predict = 128,
                stream = false // 非ストリーミング（一度に全結果を受け取る）
            };
            string jsonData = JsonConvert.SerializeObject(payload);
            ...
            {
                string jsonResponse = request.downloadHandler.text;
                var jsonResponseData = JsonConvert.DeserializeObject<JsonResponseDataDummy>(jsonResponse);
                response = jsonResponseData.content;
            }
